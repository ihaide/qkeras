...
def _round_through(x, use_stochastic_rounding=False, precision=0.5):
    ...
    if use_stochastic_rounding:
        ...
        output = x + tf.stop_gradient(-x + tf.floor(x))
        # output = x + tf.stop_gradient(-x + tf.floor(x) + tf.maximum(0.0, -tf.sign(x)))
    return output


def _round_through_legacy(x, use_stochastic_rounding=False, precision=0.5):
    """Rounds x but using straight through estimator.
...
class quantized_linear(BaseQuantizer):
    ...
    def _po2_autoscale(self, x, quantization_scale):
        ...
        quantization_scale = K.pow(
            2.0, tf.math.round(K.log(quantization_scale + K.epsilon()) / K.log(2.0))
        )
...
class quantized_hswish(quantized_bits):  # pylint: disable=invalid-name
    ...
    def __call__(self, x):
        ...
        assert self.relu_shift > 0, (
            f"relu_shift must be a positive value, " f"found {self.relu_shift} instead"
        )
        x = K.cast_to_floatx(x)
...
class quantized_exp(BaseQuantizer):  # pylint: disable=invalid-name
    """Exponentional quantizer: Quantizes an exponential function to a number of bits.
      Attributes:
      bits: number of bits to perform quantization.
      integer: number of bits to the left of the decimal point.
      use_stochastic_rounding: if true, we perform stochastic rounding.
    ...
    def __init__(
        ...
        super(quantized_exp, self).__init__()
        self.bits = bits
        self.integer = integer
        self.use_stochastic_rounding = use_stochastic_rounding
        self.exp_upper_bound = exp_upper_bound
        self.qnoise_factor = qnoise_factor
        self.use_ste = use_ste
        self.var_name = var_name
        self.use_variables = use_variables
    ...
    def __str__(self):
        ...
        if self.use_stochastic_rounding:
            flags.append(str(int(self.use_stochastic_rounding)))
        return "quantized_exp(" + ",".join(flags) + ")"

    def __call__(self, x):
        if not self.built:
            self.build(var_name=self.var_name, use_variables=self.use_variables)

        non_sign_bits = self.bits
        x = K.cast(x, dtype="float32")
        ...
        if self.exp_upper_bound is not None:
            x_u = tf.where(
                ...
                K.exp(x),
                tf.ones_like(x) * self.exp_upper_bound,
            )
        else:
            x_u = K.exp(x)
        ...
        if self.exp_upper_bound:
            xq = tf.where(
                xq <= self.exp_upper_bound, xq, tf.ones_like(xq) * self.exp_upper_bound
            )
    ...
    def max(self):
        """Get the maximum value that quantized_exp can represent."""
    ...
    def min(self):
        """Get the minimum value that quantized_exp can represent."""
        return 0.0

    def range(self):
        """Returns a list of all values that quantized_exp can represent
    ...
    def get_config(self):
        config = {
            ...
            "use_stochastic_rounding": self.use_stochastic_rounding,
            "exp_upper_bound": self.exp_upper_bound,
            "qnoise_factor": (
                self.qnoise_factor.numpy()
                if isinstance(self.qnoise_factor, tf.Variable)
                else self.qnoise_factor
