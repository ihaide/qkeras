...
from .qextralayers import QNearestNeighbourEuclidean, QNearestNeighbourManhattan
from .qextralayers import QGlobalAverageReduce1D
from .qextralayers import GlobalMaxReduce1D
from .qextralayers import QExponential
from .qextralayers import QDistanceWeighting
...
REGISTERED_LAYERS = [
    ...
    "QAdaptiveActivation",
    "QDense",
    "QNearestNeighbourEuclidean",
    "QNearestNeighbourManhattan",
    "QGlobalAverageReduce1D",
    "QExponential",
    "QDistanceWeighting",
    "GlobalMaxReduce1D",
    "QConv1D",
...
def find_bn_fusing_layer_pair(model):
    ...
    for node_id in nx.topological_sort(graph):
        ...
        if layer:
            ...
            enable_bn_fusing = (
                layer.__class__.__name__ in ["QConv2D", "QDepthwiseConv2D", "QDense"]
                and is_single
                and followed_by_bn
            )
...
def model_save_quantized_weights(model, filename=None):
    ...
    for layer in model.layers:
        if hasattr(layer, "get_quantizers"):
            ...
            if (
                isinstance(layer, QBatchNormalization)
                and layer.name in bn_layers_to_skip
            ):
            ...
            for quantizer, weight in zip(qs, ws):
                ...
                if quantizer and ("_po2" in q_name):
                    ...
                    isinstance(quantizer, quantized_bits)
                    and quantizer.alpha == "auto_po2"
                ):
...
def model_quantize(
    ...
    for layer in layers:
        if layer["class_name"].startswith("Q"):
            layer["class_name"] = layer["class_name"][1:]
        layer_config = layer["config"]

        # Dense becomes QDense, Conv1D becomes QConv1D etc
        # Activation converts activation functions.
        print(layer["class_name"], layer)
        ...
        elif (layer["class_name"] == "BatchNormalization") or (
...
def _add_supported_quantized_objects(custom_objects):
    ...
    custom_objects["QNearestNeighbourEuclidean"] = QNearestNeighbourEuclidean
    custom_objects["QNearestNeighbourManhattan"] = QNearestNeighbourManhattan
    ...
    custom_objects["QNearestNeighbourEuclidean"] = QNearestNeighbourEuclidean
    custom_objects["QNearestNeighbourManhattan"] = QNearestNeighbourManhattan
    custom_objects["QExponential"] = QExponential
    custom_objects["QDistanceWeighting"] = QDistanceWeighting
    custom_objects["QGlobalAverageReduce1D"] = QGlobalAverageReduce1D
    custom_objects["GlobalMaxReduce1D"] = GlobalMaxReduce1D
            ...
            print(
                "{}: {}".format(
                    ...
                    ", ".join(
                        [
                            "({}, {})".format(
                                weight.name, str(_get_sparsity(K.get_value(weight)))
                            )
...
def quantized_model_debug(model, X_test, plot=False, plt_instance=None):
    ...
    for n, p in zip(output_names, y_pred):
        ...
        if layer.__class__.__name__ in [
            ...
            print(
                "{:30} {: 8.4f} {: 8.4f}".format(
                    n, np.min(p / alpha), np.max(p / alpha)
                ),
                end="",
            )
        if alpha != 1.0:
            ...
            if hasattr(layer, "get_quantizers") and layer.get_quantizers()[i]:
                ...
                if i == 0 and layer.__class__.__name__ in [
                    ...
                    if plot:
                        ...
                        plt_instance.show()
            print(
                " ({: 8.4f} {: 8.4f})".format(np.min(weights), np.max(weights)), end=""
            )
        if alpha is not None and isinstance(alpha, np.ndarray):
            print(
                " a({: 10.6f} {: 10.6f})".format(np.min(alpha), np.max(alpha)), end=""
            )
        print("")
