...
from qkeras import quantized_exp
...
@pytest.mark.parametrize(
    ('bits, integer, exp_upper_bound,test_values,'
      'expected_values'), [
         (
          16, 8, 18,
          np.array([[-2,-1,0,1,3]],dtype=K.floatx()),
          np.array([[0.13525390625, 0.367919921875, 1, 2.71826171875, 18]], dtype=K.floatx()),
         ),
         (
           16, 8, None,
          np.array([[-2,-1,0,1,3]], dtype=K.floatx()),
          np.array([[0.1353352832366127, 0.36787944117144228, 1, 2.7182818284590455, 20.085536923187668]], dtype=K.floatx()),
         ),
        ])
def test_quantized_exp(bits, integer, exp_upper_bound, test_values,
                       expected_values):
  x = K.placeholder(ndim=2)
  f = K.function(
      [x], [quantized_exp(bits, integer, exp_upper_bound=exp_upper_bound)(x)])
  result = f([test_values])[0]
  assert_allclose(result, expected_values, rtol=2e-02)
